...
// Também relê o custo dos que já têm, quando passa de ?refreshDays=N dias (padrão 7): é assim
// que uma entrada de estoque na Olist alcança produto que não vende há semanas.
...
async function handle(request: Request) {
  ...
  try {
    ...
    const rd = url.searchParams.get("refreshDays")
    const refreshDays = rd === null ? 7 : Math.max(0, Number(rd) || 0)
    return NextResponse.json(await runBackfillListingCosts({ limit, refreshDays }))
  } catch (err) {
